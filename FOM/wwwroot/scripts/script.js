...
document.addEventListener('readystatechange', event => {
	...
	if (event.target.readyState === "complete") {
		if (adblockerActive) plausible('adblock');
		if (!adblockerActive) plausible('clean');
	}
});
...
if (u(".w3-dropdown-click").length) {
	u(".w3-dropdown-click").on("click", function (e) {
		e.stopPropagation();
...
let autocompleteDelay = 300 //ms
let autocompleteTimeout = null
if (document.getElementById('autocomplete').length) {
	new Autocomplete('#autocomplete', {
		search: input => {
			// const url = `${wikiUrl}/w/api.php?${params}&srsearch=${encodeURI(input)}`;
			// implement auto complete delay

			// const url = `${wikiUrl}/w/api.php?action=opensearch&format=json&search=${encodeURI(input)}`;
...
u('a[data-token]').each((item, index) => {
	...
	if (u(item).attr('data-token').length) {
		u(item).handle("click", function () {
			...
			// if(adblockerActive) {
			// 	let url = u(this).attr('href');
			// 	if(url.includes("itorrents") || url.includes("magnet")) return openAdblockPop(url)
			// 	// alert("Hello Visitor. Please Turn Off Your Ad Blocker. We Never Place Any Intrusive Ads which effects user experience much. You'll get much better Browsing Experience with your Ad-Blocker turned off 😊😊.")
			// }
			if (isChrome) dsholijehgakrhgiaerhgligurasblugikews(this);
			...
			fetch("/ping", {
				...
				body: JSON.stringify({ token: u(this).attr('data-token') })
			}).then((resp) => {
				if (!isChrome) dsholijehgakrhgiaerhgligurasblugikews(this);
			}).catch((resp) => {
				if (!isChrome) dsholijehgakrhgiaerhgligurasblugikews(this);
...
ready(function () {
	if (u('input[name=pingtoken]').length) {
		fetch("/ping", {
			...
			body: JSON.stringify({ token: u('input[name=pingtoken]').attr('value') })
		}).then((resp) => {
...
function postToken(token) {
	fetch("{{ urlPath }}/rate", {
		...
		body: JSON.stringify({ token, rating: action })
	}).then((resp) => {
		location.reload()
	})
}
...
try {
	...
	deleteCookie("message")   // Uncomment after testing is over.
} catch (e) {
...
try {
	let adblockPopInterval
	function openAdblockPop(url) {
		let timer = document.querySelector("#timer")
		let link = document.querySelector("#actual-link")
		let adBlockPop = document.querySelector("#ad-block-popup")

		let count = 30;
		adBlockPop.style.display = "block"
		link.style.display = "none"
		u("#your-link").attr("href", "")

		adblockPopInterval = setInterval(() => {
			count--;
			timer.innerHTML = `Wait ${count} sec`
			if (count == 0) {
				clearInterval(adblockPopInterval);
				timer.style.display = "none";
				link.style.display = "block"
				u("#your-link").attr("href", url)
			}
		}, 1000)
	...
	function closeAdblockPop() {

		let timer = document.querySelector("#timer")
		let link = document.querySelector("#actual-link")
		let adBlockPop = document.querySelector("#ad-block-popup")
		let count = 30;
		adBlockPop.style.display = "none"
		link.style.display = "none"
		u("#your-link").attr("href", "")
		clearInterval(adblockPopInterval);
